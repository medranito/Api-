...
async function searchPokemon() {
  ...
  try {
    ...
    const data = await res.json();
    displayPokemon([data]); // Ya muestra todo (con click para detalles)
  } catch (error) {
    alert(error.message);
  }
}
...
async function showPokemonDetails(pokemon) {
  ...
  detailsSection.innerHTML = `
    <h2>Detalles de ${capitalize(pokemon.name)}</h2>
    <img src="${pokemon.sprites.front_default}" alt="${pokemon.name}" />
    <p><strong>ID:</strong> ${pokemon.id}</p>
    <p><strong>Nombre:</strong> 
      <a href="https://www.pokemon.com/el/pokedex/${pokemon.name}" target="_blank">
    ...
    <p><strong>Altura:</strong> ${pokemon.height}</p>
    <p><strong>Habilidades:</strong> ${pokemon.abilities.map(a => a.ability.name).join(', ')}</p>
    <p><strong>Evoluciones:</strong> ${evolutions.map(e => capitalize(e)).join(' → ')}</p>
